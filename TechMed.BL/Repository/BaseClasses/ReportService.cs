...
namespace TechMed.API.Services
{
    public class ReportService : IReportService
    {
        ...
        public ReportService(IPatientCaseRepository patientCaeRepository, TeleMedecineContext teleMedecineContext)
        {
            _patientCaeRepository = patientCaeRepository;
            //_converter = converter;
            _teleMedecineContext = teleMedecineContext;
        }
        public async Task<ReportDTO> GeneratePdfReport(Int64 PatientCaseID, string contentRootPath)
        {
            ReportDTO reportDTO = new ReportDTO();
            long CheckID = DateTime.Now.Ticks;
            var Results = _teleMedecineContext.UpdatePrescriptionDocumentFlag.FromSqlInterpolated($"EXEC [dbo].[UpdatePrescriptionDocumentFlag] @CaseID ={PatientCaseID}, @RequestID ={CheckID}");
            UpdatePrescriptionDocumentFlagVM data;
            DateTime dt = new DateTime(CheckID);
            foreach (var item in Results)
            {
                if (item.Prescription.ToLower().Contains(".pdf"))
                {
                    reportDTO.PrescriptionFilePath = item.Prescription;
                    return reportDTO;
                }
                else if (item.Prescription == "consultationpending")
                {
                    reportDTO.PrescriptionFilePath = "consultationpending";
                    return reportDTO;
                }
                else if (item.Prescription != "pending" + CheckID.ToString())
                {
                    reportDTO.PrescriptionFilePath = "pending";
                    return reportDTO;
                }
            ...
            try
            {
                ...
                //if (!string.IsNullOrEmpty(patientCaseVM.Prescription))
                //{
                //    reportDTO.PrescriptionFilePath = patientCaseVM.Prescription;
                //    return reportDTO;
                //}
                ...
                using (var ms = new MemoryStream(testFileBytes))
                {
                    ...
                    string filepath = "";
                    var result = _patientCaeRepository.UploadCaseDocFromByte(caseDocuments, contentRootPath, out filepath);
                    reportDTO.PrescriptionFilePath = filepath;
                }
                //return _converter.Convert(htmlToPdfDocument);
                reportDTO.PrescriptionFile = testFileBytes;
            }
            catch (Exception)
            {
                PatientCase patientCase = await _teleMedecineContext.PatientCases.FirstOrDefaultAsync(a => a.Id == PatientCaseID);
                if (patientCase != null)
                {
                    if (patientCase.Prescription != null)
                    {
                        if (!patientCase.Prescription.ToLower().Contains(".pdf"))
                        {
